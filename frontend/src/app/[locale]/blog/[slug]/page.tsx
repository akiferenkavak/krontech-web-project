...
export default async function BlogDetailPage({
  ...
  params: Promise<{ locale: Locale; slug: string }>;
  searchParams: Promise<{ previewMode?: string }>;
}) {
  const { locale, slug } = await params;
  const { previewMode } = await searchParams;
  const isTr = locale === 'tr';

  // Draft mode kontrolü
  const { isEnabled: isPreview } = await draftMode();

  // Preview modda backend'e ?preview=true gönder — DRAFT içerikler de gelsin
  const isPreviewMode = isPreview && previewMode === 'true';
  ...
  try {
    post = await getBlogPostBySlug(slug, locale, isPreviewMode);
  } catch { post = null; }
    ...
    <>
      ...
      {isPreviewMode && (
        <div style={{
          ...
          <span>
            ⚠️ {isTr
              ? `Önizleme modu — durum: ${translation?.status ?? 'bilinmiyor'}`
              : `Preview mode — status: ${translation?.status ?? 'unknown'}`}
          </span>
          <a
